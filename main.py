...
広告ごとに 日本語→中国語→英語→ロシア語→フランス語 の順で出力
"""
...
def format_ads_text_by_platform(ads_dict: dict) -> str:
    platforms = [
        ("x", "X"),
        ("fb", "Facebook"),
        ("tt", "TikTok"),
...
async def process_photos(chat_id: int, file_ids: list):
    try:
        ...
        full_text = (
            ...
            f"型式: {model_code}\n\n"
            f"【車情報】\n{car_info}\n\n"
            f"{format_ads_text_by_platform(ads_dict)}"
        )

        if len(full_text) <= 3500:
            await send_message(chat_id, full_text)
        else:
            await send_message(
                chat_id,
                f"✅ 完了\n型式: {model_code}\n\n【車情報】\n{car_info}"
            ...
            body_text = format_ads_text_by_platform(ads_dict)
            chunk_size = 3000
    ...
    if "photo" in message:
        ...
        if text in ["/START", "START", "/HELP"]:
            await send_message(
                ...
                "（オークションシート＋車体写真）\n\n"
                "【出力】\n"
                "広告ごとに、日本語 → 中国語 → 英語 → ロシア語 → フランス語 の順で返します。\n\n"
